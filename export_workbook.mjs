...
if (!inputPath || !outputPath) throw new Error("usage: node export_workbook.mjs input.json output.xlsx [preview-dir]");
const data = JSON.parse(await fs.readFile(inputPath, "utf8"));
const wb = Workbook.create();
wb.comments.setSelf({ displayName: "User" });
const cover = wb.worksheets.add("Cover");
const inputs = wb.worksheets.add("Source Data");
const summary = wb.worksheets.add("QoE Summary");
const wc = wb.worksheets.add("Working Capital");
const candidates = wb.worksheets.add("Review Candidates");
const audit = wb.worksheets.add("Audit Trail");
...
cover.getRange("D5:J9").merge(); cover.getRange("D5").values = [["공시상 이익 증가가 지속 가능한 영업성과인지 확인하기 위해 정상화 조정 후보, 운전자본과 현금전환을 함께 검토합니다. 자동으로 기업가치나 정상화 이익을 확정하지 않으며, 거래 검토자가 원문을 확인할 항목과 계산 근거를 빠르게 찾도록 설계한 PoC입니다."]];
...
cover.getRange("A15:J18").merge(); cover.getRange("A15").values = [["1) QoE Summary에서 이익·현금흐름 추이를 확인합니다.  2) Working Capital에서 회전일수와 매출 대비 운전자본 변화를 봅니다.  3) Review Candidates에서 자동 탐지 항목의 원문을 확인하고 사용자 조정 여부·사유를 기록합니다.  4) Audit Trail과 Checks로 출처·산식·오류를 검토합니다."]];
...
title(inputs, "Source Data | 공시 추출값", "O");
...
inputs.getRange(`A5:O${4+data.metrics.length}`).values = data.metrics.map(m => [m.year,m.revenue,m.operating_profit,m.net_income,m.cfo,m.ar,m.inventory,m.ap,m.cash,m.cogs,m.debt,m.lease,"DART 연결 재무제표 API","Y",""]);
...
title(summary, "QoE Summary | 보고이익·현금전환·순차입금", "F");
summary.getRange("A4").values = [["지표"]];
summary.getRange("A5:A14").values = [["매출액"],["매출 성장률"],["영업이익"],["영업이익률"],["영업현금흐름"],["영업이익 대비 CFO"],["순이익"],["순이익–CFO 괴리"],["순차입금"],["검토 후보 수"]];
...
data.years.forEach((y,i) => {
  ...
  summary.getRange(`${c}5:${c}14`).formulas = [[`='Source Data'!B${src}`],[i?`=IFERROR(${c}5/${String.fromCharCode(65+i)}5-1,"")`:""],[`='Source Data'!C${src}`],[`=IFERROR(${c}7/${c}5,"")`],[`='Source Data'!E${src}`],[`=IFERROR(${c}9/${c}7,"")`],[`='Source Data'!D${src}`],[`=${c}11-${c}9`],[`='Source Data'!K${src}+IF(Cover!$B$10="순차입금에 포함",'Source Data'!L${src},0)-'Source Data'!I${src}`],[`=COUNTIF('Review Candidates'!$A$5:$A$204,${c}$4)`]];
...
summary.getRange("A15:F15").merge(); summary.getRange("A15").values = [["해석 주의: CFO/영업이익 저하, 순이익–CFO 괴리 확대, 순차입금 증가는 결론이 아니라 추가 질문의 출발점입니다."]]; summary.getRange("A15:F15").format = { fill: amber, wrapText:true };
// Chart helper block uses formulas so source edits flow through.
summary.getRange("H4:J4").values = [["연도","영업이익률","CFO/영업이익"]];
...
title(wc, "Working Capital | 회전일수와 순운전자본", "F");
wc.getRange("A4").values = [["지표"]];
wc.getRange("A5:A12").values = [["매출액"],["매출채권회전일수"],["재고자산회전일수"],["매입채무회전일수"],["현금전환주기(참고)"],["순운전자본"],["매출 대비 NWC"],["AR+재고 증가율 vs 매출 증가율"]];
...
data.years.forEach((_,i)=>{ const c=String.fromCharCode(66+i), src=5+i, prev=String.fromCharCode(65+i);
  wc.getRange(`${c}5:${c}12`).formulas=[[`='Source Data'!B${src}`],[`=IFERROR(AVERAGE('Source Data'!F${Math.max(5,src-1)}:'Source Data'!F${src})/'Source Data'!B${src}*365,"")`],[`=IFERROR(AVERAGE('Source Data'!G${Math.max(5,src-1)}:'Source Data'!G${src})/'Source Data'!J${src}*365,"")`],[`=IFERROR(AVERAGE('Source Data'!H${Math.max(5,src-1)}:'Source Data'!H${src})/'Source Data'!J${src}*365,"")`],[`=${c}6+${c}7-${c}8`],[`='Source Data'!F${src}+'Source Data'!G${src}-'Source Data'!H${src}`],[`=IFERROR(${c}10/${c}5,"")`],[i?`=IFERROR((('Source Data'!F${src}+'Source Data'!G${src})/('Source Data'!F${src-1}+'Source Data'!G${src-1})-1)-(${c}5/${prev}5-1),"")`:""]];
});
...
title(candidates, "Review Candidates | 정상화 조정 후보 (결론 아님)", "L");
...
const candRows=(data.candidates||[]).slice(0,200).map(x=>[x.year,x.category,x.account,x.amount,x.excerpt,x.rcept_no||"",x.rcept_no?`https://dart.fss.or.kr/dsaf001/main.do?rcpNo=${x.rcept_no}`:"",x.source,"Y",x.status,"미결정",""]);
...
title(audit,"Audit Trail | 사용 공시·계정·산식", "J");
audit.getRange("A4:J4").values=[["연도","사용 공시","접수일","접수번호","원문 URL","재무제표 기준","사용 계정","계산식","자동 추출","비고"]]; header(audit.getRange("A4:J4"));
...
audit.getRange(`A5:J${4+data.years.length}`).values=data.years.map(y=>{const f=filingMap[y]||{};return[y,f.report_nm||"사업보고서",f.rcept_dt||"",f.rcept_no||"",f.url||"",data.metadata.basis,"Source Data 시트 참조","각 분석 시트의 셀 수식 참조","Y",""];});
audit.getRange("A11:J11").merge(); audit.getRange("A11").values=[["산식 정의"]]; header(audit.getRange("A11:J11"));
audit.getRange("A12:B19").values=[["영업이익률","영업이익 / 매출액"],["CFO/영업이익","영업활동현금흐름 / 영업이익"],["AR Days","평균 매출채권 / 매출액 × 365"],["Inventory Days","평균 재고자산 / 매출원가 × 365"],["AP Days","평균 매입채무 / 매출원가 × 365"],["순운전자본","매출채권 + 재고자산 - 매입채무"],["순차입금","차입금·사채 + 선택 시 리스부채 - 현금"],["정상화 후보","계정명·원문 키워드 자동 탐지 후 사용자 판단"]];
audit.freezePanes.freezeRows(4); widths(audit,{A:9,B:27,C:13,D:18,E:45,F:18,G:26,H:42,I:12,J:35});

title(checks,"Checks | 완전성·산식·한계", "G");
checks.getRange("A4:G4").values=[["Check","Actual","Expected","Difference","Tolerance","Status","Notes"]]; header(checks.getRange("A4:G4"));
checks.getRange("A5:G9").values=[
  ["분석기간 수",data.years.length,"3개년 권장",0,0,data.years.length>=2?"OK":"REVIEW","MVP는 3개 사업연도 권장"],
  ["연결재무제표",data.metadata.basis,"연결재무제표",0,0,data.metadata.basis.includes("연결")?"OK":"REVIEW","연결 우선 원칙"],
  ["매출 누락",data.metrics.filter(x=>x.revenue==null).length,0,data.metrics.filter(x=>x.revenue==null).length,0,data.metrics.every(x=>x.revenue!=null)?"OK":"REVIEW","Source Data 확인"],
  ["영업이익 누락",data.metrics.filter(x=>x.operating_profit==null).length,0,data.metrics.filter(x=>x.operating_profit==null).length,0,data.metrics.every(x=>x.operating_profit!=null)?"OK":"REVIEW","Source Data 확인"],
  ["원문 수집 오류",(data.errors||[]).length,0,(data.errors||[]).length,0,(data.errors||[]).length===0?"OK":"REVIEW","오류는 제외하지 않고 표시"],
];
checks.getRange("F5:F9").conditionalFormats.add("containsText",{text:"OK",format:{fill:green,font:{bold:true,color:"#006100"}}});
checks.getRange("F5:F9").conditionalFormats.add("containsText",{text:"REVIEW",format:{fill:red,font:{bold:true,color:"#9C0006"}}});
...
const inspection=await wb.inspect({kind:"table",range:"QoE Summary!A1:F15",include:"values,formulas",tableMaxRows:20,tableMaxCols:8});
...
if(previewDir){await fs.mkdir(previewDir,{recursive:true}); for(const s of ["Cover","QoE Summary","Working Capital","Review Candidates","Audit Trail","Checks"]){const blob=await wb.render({sheetName:s,autoCrop:"all",scale:1,format:"png"}); await fs.writeFile(`${previewDir}/${s.replaceAll(" ","_")}.png`,new Uint8Array(await blob.arrayBuffer()));}}
