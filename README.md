...
- 손상차손, 처분손익, 구조조정비용 등 보고이익에서 별도 검토할 항목이 포함되어 있지 않은가?
...
정상화 영업이익에 직접 반영할 가능성이 높은 다음 4개 유형만 손익계산서 계정명과 사업보고서 원문 주석으로 탐색합니다.

1. 유형자산 처분손익
2. 자산 손상차손과 환입
3. 소송·재해 등 사건
4. 구조조정·희망퇴직·기업결합 등 거래 관련 비용

재무제표 API 후보는 손익계산서·포괄손익계산서의 금액 있는 계정만 사용합니다. 원문 주석 후보는 근처의 표시단위와 같은 표 행의 금액을 함께 찾은 경우에만 원 단위로 환산하여 제시합니다. 금액을 찾지 못한 목차·회계정책 키워드와 완전히 중복된 후보는 제외합니다. 추출 금액은 양수로 표시하고, `손익 방향(자동)`에서 이익·손실 여부만 제시합니다.

재고자산평가손실·대손상각비·품질보증충당부채·정부보조금·관계기업·지분법·일반 기타수익·기타비용은 반복 가능하거나 영업이익 밖의 항목이므로 정상화 손익 후보에서 추출하지 않습니다.

탐지된 강한 일회성 후보는 Excel에서 `사용자 판단=일회성`, `정상화 대상=영업이익`, `조정 여부=예`로 초기 반영됩니다. 따라서 일회성 손실은 보고 영업이익에 가산되고 일회성 이익은 차감되어 정상화 영업이익이 즉시 계산됩니다. 다만 자동 탐지 결과는 확정 조정액이 아니므로 원문 확인 결과 반복 항목이거나 보고 영업이익에 포함되지 않았다면 사용자가 `아니요` 또는 `제외`로 변경해야 합니다.

Excel의 `검토 후보` 시트에서 사용자가 다음 항목을 입력하면 `정상화 손익` 시트와 `QoE 요약`이 자동으로 다시 계산됩니다.

1. 자동 반영된 손익 방향·금액·원문을 확인
2. 반복 항목이면 `사용자 판단=반복`과 `조정 여부=아니요`로 변경
3. 보고 영업이익에 포함되지 않은 항목이면 `정상화 대상=제외` 또는 `조정 여부=아니요`로 변경
4. 적용 금액과 조정 사유 입력

정상화 영업이익에는 `사용자 판단=일회성`, `정상화 대상=영업이익`, `조정 여부=예`이고 손익 방향이 확인된 항목만 반영됩니다. 손실은 가산하고 이익은 차감합니다.

상단 `전체 조정 여부`의 기본값은 `예`이며 모든 강한 일회성 후보를 정상화 영업이익에 반영합니다. `아니요`를 선택하면 전체 후보를 한 번에 제외할 수 있습니다. `검증` 시트는 조정 여부가 `예`이지만 반영 조건을 충족하지 못한 항목 수를 표시합니다.

`정상화 손익`과 `QoE 요약`은 확정 조정액과 함께 `정상화 판단 미완료 후보 수`를 표시합니다. 따라서 조정액이 `-`여도 미완료 후보가 남아 있으면 “조정사항 없음”이 아니라 원문 확인과 사용자 판단이 남아 있는 상태입니다.
...
7. 생성된 Excel의 `검토 후보`에서 반복성·정상화 대상·조정 여부·적용 금액과 사유를 입력합니다.
...
| `정상화 손익` | 손익 방향·반복성·정상화 대상 검토, 가산·차감 조정액, 정상화 영업이익과 이익률 |
| `운전자본` | 회전일수, 현금전환주기, 순운전자본과 검토 포인트 |
| `순차입금` | 현금·차입금·리스부채 구성, 순차입금·순현금과 전년 대비 증감 |
| `검토 후보` | 유형, 계정·키워드, 금액, 원문 발췌, 반복성 힌트와 사용자 판단란 |
...
| 정상화 순조정액 | `일회성·영업이익·조정 예`로 확인된 손실 가산액 - 이익 차감액 |
