...
다음 6개 유형을 손익계산서 계정명과 사업보고서 원문 주석으로 탐색합니다.
...
탐지된 항목은 **확정 조정액이 아니라 확인이 필요한 후보**입니다. 특히 재고자산평가손실·대손상각비·품질보증충당부채·정부보조금은 매년 반복될 수 있으므로 자동으로 일회성 처리하지 않습니다. 유형자산 처분손익도 처분 정책과 반복 실적을 확인해야 합니다.

Excel의 `검토 후보` 시트에서 사용자가 다음 항목을 입력하면 `정상화 손익` 시트와 `QoE 요약`이 자동으로 다시 계산됩니다.

1. 자동 제시된 손익 방향과 반복성 힌트를 원문·과거 공시로 확인
2. `사용자 판단`을 `일회성`, `반복`, `미결정` 중에서 선택
3. `정상화 대상`을 `영업이익`, `순이익`, `제외`, `미결정` 중에서 선택
4. 필요한 후보의 `조정 여부`를 `예` 또는 `아니요`로 선택
5. 적용 금액과 조정 사유 입력

정상화 영업이익에는 `사용자 판단=일회성`, `정상화 대상=영업이익`, `조정 여부=예`이고 손익 방향이 확인된 항목만 반영됩니다. 손실은 가산하고 이익은 차감합니다. 관계기업·지분법·금융손익·중단영업처럼 영업이익 밖의 항목은 `순이익` 검토 대상으로 분리하며 현재 PoC의 정상화 영업이익에는 반영하지 않습니다.

상단 `전체 조정 여부`는 조정 여부만 한 번에 바꾸며, 반복성과 정상화 대상을 자동 확정하지 않습니다. `검증` 시트는 조정 여부가 `예`이지만 위 조건을 충족하지 못한 항목 수를 표시합니다.
