...
1. 사용자 조정 여부
2. 조정 방향: 일회성 손실 제거는 `가산`, 일회성 이익 제거는 `차감`
3. 적용 금액
4. 조정 사유

정상화 영업이익은 `보고 영업이익 + 가산 조정액 - 차감 조정액`으로 계산합니다. 미결정 후보는 자동으로 제외하지 않으며, 자동 추출 금액은 사용자가 확인할 참고값입니다.
...
7. 생성된 Excel의 `검토 후보`에서 조정 여부·방향·적용 금액과 사유를 입력합니다.
...
| `정상화 손익` | 일회성 손익 후보, 가산·차감 조정액, 정상화 영업이익과 이익률 |
...
| 정상화 순조정액 | 사용자가 선택한 일회성 손실 가산액 - 일회성 이익 차감액 |
