...
다음 5개 유형을 손익계산서 계정명과 사업보고서 원문 주석으로 탐색합니다.
...
재무제표 API 후보는 손익계산서·포괄손익계산서의 금액 있는 계정만 사용합니다. 원문 주석 후보는 근처의 표시단위와 같은 표 행의 금액을 함께 찾은 경우에만 원 단위로 환산하여 제시합니다. 금액을 찾지 못한 목차·회계정책 키워드와 완전히 중복된 후보는 제외합니다. 추출 금액은 양수로 표시하며, 조정 방향은 `손익 구분(자동)`에 따라 일회성 이익은 차감하고 일회성 손실은 가산합니다.
...
1. 상단 `전체 조정 여부`에서 모든 후보를 한 번에 `예` 또는 `아니요`로 변경
2. 필요한 후보의 조정 여부를 개별적으로 `예` 또는 `아니요`로 수정
3. 적용 금액
4. 조정 사유
